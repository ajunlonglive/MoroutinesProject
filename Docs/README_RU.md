...
### Объект `MoroutinesDefaultOwner`
Перед самым запуском вашей игры в сцене будет создан объект `MoroutinesDefaultOwner`, который будет изолирован в `DontDestroyOnLoad` сцену и скрыт в ней, так что вы не заметите его. Фактическое выполнение <b>всех</b> морутин происходит на этом объекте. Однако хозяином может быть кто угодно. У безхозных морутин хозяином является `MoroutinesDefaultOwner`.
