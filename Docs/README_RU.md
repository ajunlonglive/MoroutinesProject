...
> ВАЖНО! Если морутина более не используется в вашей игре, то она должна быть уничтожена (не важно как, автоматически, или вручную с помощью метода `Destroy`), иначе память не будет освобождена.
...
mor.SetOwner(otherGameObject); // установить другого владельца.
mor.MakeUnowned(); // сделать морутину бесхозной.
```
